...
public class MavlinkService : BackgroundService
{
    ...
    private readonly FirmwareProfileService _firmware;
    ...
    private ushort[] _prevRcChannels = new ushort[18];      // channels from prior packet
    ...
    private const int RcFrozenThreshold = 6000;            // ~10 minutes at 10Hz to prevent false disconnects on digital/sbus systems
    ...
    public ushort[] RcMin { get; private set; } = new ushort[18];
    public ushort[] RcMax { get; private set; } = new ushort[18];
    ...
    private float _px4BatVEmpty = 3.5f;
    private float _px4BatVCharged = 4.2f;
    private int _px4FsRecv = 0;


    public MavlinkService(ILogger<MavlinkService> logger, IConfiguration config, VehicleProfileService vehicleProfile, FirmwareProfileService firmware)
    {
        _logger = logger;
        _config = config;
    ...
    private void WipeDataToBlankSlate()
    {
        ...
        Array.Clear(_prevRcChannels, 0, 18);
        _rcFrozenCount = 0;
        _rcLiveCount = 0;
        IsLinkHealthy = false;
        _lastGpsFix = -1;
    ...
    private void HandlePacket(MAVLink.MAVLinkMessage packet)
    {
        ...
        if (packet.msgid == (uint)MAVLink.MAVLINK_MSG_ID.HEARTBEAT)
        {
            ...
            {
                if (_firmware.UpdateFromHeartbeat(hb.autopilot, hb.type))
                    _logger.LogInformation("[FIRMWARE] Auto-detected {Firmware}", _firmware.ActiveFirmwareDisplayName);
            }
            ...
            if (!_gotFirstHeartbeatThisSession && _consecutiveHeartbeatCount >= requiredHeartbeats)
            {
                ...
                // Request firmware-specific bootstrap params on connect
                _ = Task.Run(async () =>
                {
                    await Task.Delay(800);
                    var bootstrap = _firmware.ActiveAdapter.GetConnectBootstrapParams(_vehicleProfile.ActiveProfile);
                    foreach (var param in bootstrap)
                    {
                        RequestParamRead(param);
                        await Task.Delay(IsWireless ? 80 : 20);
                    }

                    // ArduPilot-only: retry failsafe/battery params missed on lossy radio links
                    if (_firmware.ActiveFirmware == FirmwareFamily.ArduPilot)
                    {
                    ...
                    else if (_firmware.ActiveFirmware == FirmwareFamily.PX4)
                    {
                        await Task.Delay(IsWireless ? 4000 : 1500);
                        if ((_fsRecv & 0b0001) == 0) { RequestParamRead("NAV_RCL_ACT"); await Task.Delay(IsWireless ? 80 : 20); }
                        if ((_fsRecv & 0b0010) == 0) { RequestParamRead("COM_RC_LOSS_T"); await Task.Delay(IsWireless ? 80 : 20); }
                        if ((_fsRecv & 0b0100) == 0) { RequestParamRead("COM_LOW_BAT_ACT"); await Task.Delay(IsWireless ? 80 : 20); }
                        if ((_fsRecv & 0b1000) == 0) { RequestParamRead("NAV_DLL_ACT"); await Task.Delay(IsWireless ? 80 : 20); }
                        if ((_battRecv & 0x02) == 0) { RequestParamRead("BAT1_CAPACITY"); await Task.Delay(IsWireless ? 80 : 20); }
                        if ((_battRecv & 512) == 0) { RequestParamRead("BAT1_N_CELLS"); await Task.Delay(IsWireless ? 80 : 20); }
                        if ((_px4FsRecv & 1) == 0) { RequestParamRead("BAT1_V_EMPTY"); await Task.Delay(IsWireless ? 80 : 20); }
                        if ((_px4FsRecv & 2) == 0) { RequestParamRead("BAT1_V_CHARGED"); await Task.Delay(IsWireless ? 80 : 20); }
                    }
                });
            }
        }
        ...
        else if (packet.msgid == (uint)MAVLink.MAVLINK_MSG_ID.RC_CHANNELS)
        {
            ...
            UpdateState(s =>
            {
                s.RcChannelCount = chanCount;
                for (int i = 0; i < 18; i++) s.RawChannels[i] = latestChannels[i];
            });

            if (IsRcCalibrating)
            {
                for (int i = 0; i < Math.Min((int)chanCount, 18); i++)
                {
            ...
            Array.Copy(latestChannels, _prevRcChannels, 18);
        ...
        else if (packet.msgid == (uint)MAVLink.MAVLINK_MSG_ID.PARAM_VALUE)
        {
            ...
            else if (paramName.StartsWith("COM_FLTMODE") && paramName.Length == 12 && char.IsDigit(paramName[11]))
            {
                int modeIdx = paramName[11] - '1';
                if (modeIdx >= 0 && modeIdx < 6)
                {
                    _fltModeValues[modeIdx] = param.param_value;
                    _fltModeReceived |= (1 << modeIdx);
            ...
            else if (paramName == "NAV_RCL_ACT" && _firmware.ActiveFirmware == FirmwareFamily.PX4) { _fsThrEnable = (int)param.param_value; _fsRecv |= 1; CheckFireFailsafe(); }
            else if (paramName == "COM_RC_LOSS_T" && _firmware.ActiveFirmware == FirmwareFamily.PX4) { _fsThrValue = (int)(param.param_value * 1000); _fsRecv |= 2; CheckFireFailsafe(); }
            else if (paramName == "COM_LOW_BAT_ACT" && _firmware.ActiveFirmware == FirmwareFamily.PX4) { _fsBattEnable = (int)param.param_value; _fsRecv |= 4; CheckFireFailsafe(); }
            else if (paramName == "NAV_DLL_ACT" && _firmware.ActiveFirmware == FirmwareFamily.PX4) { _fsGcsEnable = (int)param.param_value; _fsRecv |= 8; CheckFireFailsafe(); }
            ...
            // ── PX4 Battery monitor params ─────────────────────────────────
            else if (paramName == "BAT1_N_CELLS" && _firmware.ActiveFirmware == FirmwareFamily.PX4)
            {
                _battNumCells = (int)param.param_value;
                _battRecv |= 512;
                _battMonitor = 4;
                _battRecv |= 1;
                UpdatePx4BatteryVoltages();
                CheckFireBattery();
            }
            else if (paramName == "BAT1_CAPACITY" && _firmware.ActiveFirmware == FirmwareFamily.PX4)
            {
                _battCapacity = (int)param.param_value;
                _battRecv |= 2;
                CheckFireBattery();
            }
            else if (paramName == "BAT1_V_EMPTY" && _firmware.ActiveFirmware == FirmwareFamily.PX4)
            {
                _px4BatVEmpty = param.param_value;
                UpdatePx4BatteryVoltages();
                CheckFireBattery();
            }
            else if (paramName == "BAT1_V_CHARGED" && _firmware.ActiveFirmware == FirmwareFamily.PX4)
            {
                _px4BatVCharged = param.param_value;
                UpdatePx4BatteryVoltages();
                CheckFireBattery();
            }
        }
    ...
    /// <summary>
/// Returns the flags bytes that would be generated for each fragment of
/// a frame of the given length and sequence number.
/// Used for unit testing fragmentation logic.
/// </summary>
public static byte[] ComputeFragmentFlags(int length, int seq)
{
    ...
    var result = new byte[nopackets];
    for (int a = 0; a < nopackets; a++)
    {
        byte f = 0;
        if (nopackets > 1) f |= 0x01;
        f |= (byte)((a       & 0x03) << 1);
        f |= (byte)((seq     & 0x1F) << 3);
        result[a] = f;
    }
...
private void SendRtcmFragments((byte[] Buf, byte Flags, byte Len)[] packets)
{
    ...
    public void StartCalibration(string type)
    {
        switch (type.ToUpper())
        {
            ...
            case "MAG":
                ...
                break;
            case "RADIO":
                for (int i = 0; i < 18; i++) { RcMin[i] = 2200; RcMax[i] = 800; }
    ...
    {
        var (p1, p2, p3) = _firmware.ActiveAdapter.GetSetModeCommandParams(modeName, _vehicleProfile.ActiveProfile);
        TryGetTarget(out var sysId, out var compId);
        var req = new MAVLink.mavlink_command_long_t
        {
            target_system = sysId,
    ...
    private static int GetPx4ModeIndex(string modeName)
    {
    ...
    private static string GetPx4ModeNameFromIndex(int index)
    {
        return index switch
        {
            0 => "MANUAL",
    ...
    {
        if (_firmware.ActiveFirmware == FirmwareFamily.PX4)
        {
            float modeValue = GetPx4ModeIndex(modeName);
            SendParameter($"COM_FLTMODE{position}", modeValue);
        }
        else
        {
    ...
    private void UpdatePx4BatteryVoltages()
    {
        if (_battNumCells > 0)
        {
            _battLowVolt = _px4BatVEmpty * _battNumCells;
            _battCrtVolt = _px4BatVCharged * _battNumCells;
            _battRecv |= 4; // BATT_LOW_VOLT bit
            _battRecv |= 8; // BATT_CRT_VOLT bit
        }
    }
    ...
    private void CheckFireBattery()
    {
        if (_firmware.ActiveFirmware == FirmwareFamily.PX4)
        {
            // For PX4, we only need cells, capacity, empty and charged voltages (bits 1, 2, 4, 8, 512).
            if ((_battRecv & 527) == 527)
        ...
        double voltMult, double ampPerVlt, double ampOffset, int numCells)
    {
        if (_firmware.ActiveFirmware == FirmwareFamily.PX4)
        {
            SendParameter("BAT1_CAPACITY", capacity);
            SendParameter("BAT1_N_CELLS", numCells);
            if (numCells > 0)
            {
                SendParameter("BAT1_V_EMPTY", (float)(lowVolt / numCells));
                SendParameter("BAT1_V_CHARGED", (float)(crtVolt / numCells));
            }
        }
        else
        {
    ...
    public void SaveFailsafeParams(int thrEnable, int thrValue, int battEnable, int gcsEnable)
    {
        if (_firmware.ActiveFirmware == FirmwareFamily.PX4)
        {
            SendParameter("NAV_RCL_ACT", thrEnable);
            SendParameter("COM_RC_LOSS_T", (float)(thrValue / 1000.0));
            SendParameter("COM_LOW_BAT_ACT", battEnable);
            SendParameter("NAV_DLL_ACT", gcsEnable);
        }
        else
        {
    ...
    private void FireFlightModeParams()
    {
        ...
        if (_firmware.ActiveFirmware == FirmwareFamily.PX4)
        {
            names = _fltModeValues.Select(v => GetPx4ModeNameFromIndex((int)v)).ToArray();
        }
        else
        {
            names = _fltModeValues.Select(v => GetFlightModeString((uint)v)).ToArray();
        }
