...
public sealed class VehicleProfileService
{
    ...
    public string ActiveProfileDisplayName
    {
        get
        {
            var isPx4 = _firmware.ActiveFirmware == FirmwareFamily.PX4;
            return _activeProfile switch
            {
                VehicleProfile.ArduRover  => isPx4 ? "PX4 Rover" : "ArduRover",
                VehicleProfile.ArduCopter => isPx4 ? "PX4 Multirotor" : "ArduCopter",
                _                         => "Unknown"
            };
        }
    }
    ...
    public VehicleProfileService(FirmwareProfileService firmware) => _firmware = firmware;
    ...
    public string GetFlightModeString(uint customMode)
    {
        lock (_lock)
        {
            return _firmware.ActiveAdapter.GetFlightModeString(customMode, _activeProfile);
        }
    }
    ...
    public float GetModeValue(string modeName)
    {
        lock (_lock)
        {
            return _firmware.ActiveAdapter.GetModeValue(modeName, _activeProfile);
        }
    }
    ...
    public IReadOnlyList<string> GetAvailableModes()
    {
        lock (_lock)
        {
            return _firmware.ActiveAdapter.GetAvailableModes(_activeProfile);
        }
    }
